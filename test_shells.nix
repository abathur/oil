...
rec {
  ...
  test_dash = pkgs.dash.overrideAttrs (oldAttrs: rec {
    ...
    postInstall = if pkgs.glibcLocales != null then ''
      wrapProgram "$out/bin/dash" --run "export LOCALE_ARCHIVE='${pkgs.glibcLocales}/lib/locale/locale-archive' LC_CTYPE='C.UTF-8'"
      substituteInPlace $out/bin/dash --replace "${pkgs.bash}/bin/bash" "${bang_bash}/bin/bash"
  ...
  test_bash = pkgs.bash.overrideAttrs (oldAttrs: with pkgs; rec{
  ...
  test_mksh = pkgs.mksh.overrideAttrs (oldAttrs: rec {
    ...
    buildInputs = [ pkgs.makeWrapper ];
    preFixup = if pkgs.glibcLocales != null then ''
      wrapProgram "$out/bin/mksh" --run "export LOCALE_ARCHIVE='${pkgs.glibcLocales}/lib/locale/locale-archive' LC_CTYPE='C.UTF-8'"
      substituteInPlace $out/bin/mksh --replace "${pkgs.bash}/bin/bash" "${bang_bash}/bin/bash"
  ...
  test_zsh = pkgs.zsh.overrideAttrs (oldAttrs: rec {
    ...
    buildInputs = oldAttrs.buildInputs ++ [ pkgs.makeWrapper ];
    postInstall = if pkgs.glibcLocales != null then ''
      wrapProgram "$out/bin/zsh" --run "export LOCALE_ARCHIVE='${pkgs.glibcLocales}/lib/locale/locale-archive' LC_CTYPE='C.UTF-8'"
      substituteInPlace $out/bin/zsh --replace "${pkgs.bash}/bin/bash" "${bang_bash}/bin/bash"
    '' else "" + (oldAttrs.postInstall or "");
  });
}
